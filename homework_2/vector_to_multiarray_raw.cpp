...
void fill_multi_array(const Container & container, Forward_Iterator multi_array)
{
    ...
    if constexpr (N > 1) {
        for (auto i = 0U; i < size; i++) {
            fill_multi_array<N - 1>(*(std::next(std::begin(container), i)),
                    std::next(multi_array, i)->begin());
        }
    }
    else {
        for (auto i = 0U; i < size; i++) {
            *std::next(multi_array, i) = *std::next(std::begin(container), i);
        }
    }


	// Enjoy debugging! Actually, it was pretty fast :)
}
